...
const { regionOf, groupByRegion, buildGroups } = await import('../src/groups.js');

describe('regionOf', () => {
  it('identifies HK', () => assertEq(regionOf('🇭🇰香港专线01|BGP|住宅IP'), 'HK'));
  it('identifies SG', () => assertEq(regionOf('🇸🇬新加坡专线01|BGP|流媒体'), 'SG'));
  it('identifies JP', () => assertEq(regionOf('🇯🇵日本专线01|BGP|流媒体'), 'JP'));
  it('identifies KR', () => assertEq(regionOf('🇰🇷韩国专线01|BGP|流媒体'), 'KR'));
  it('identifies TW', () => assertEq(regionOf('🇨🇳台湾专线01|BGP|流媒体'), 'TW'));
  it('identifies UK', () => assertEq(regionOf('🇬🇧英国伦敦01|流媒体|0.1x'), 'UK'));
  it('identifies UK by 英国', () => assertEq(regionOf('🇬🇧英国伦敦02|流媒体|0.1x'), 'UK'));
...
describe('buildGroups', () => {
  it('generates all group lines', () => {
    ...
    assert(output.includes('Bilibili = select,DIRECT,HK,TW'), 'has Bilibili');
  });
});